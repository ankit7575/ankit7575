...
const Stage3 = () => {
  ...
  useEffect(() => {
    const loadUserData = () => {
      ...
      try {
        ...
        // Filter referrals based only on matching referralbyIdStage3
        const filteredReferrals = data.admin.users
          .filter(user => user.referralbyIdStage3 === mainUser.referralId) // Match only referralbyIdStage3
  ...
  return (
    <Layout userRole={userProfile?.role}> {/* Pass userRole to Layout */}
      <section className="stage3-referrals">
        <h1>Stage 3 Referral Overview</h1>
        <div className="referral-summary">
          <h2>Your Stage 3 Referrals</h2>
          <table className="referral-table">
            ...
            <tbody>
              {referrals.length > 0 ? (
                ...
                <tr>
                  <td colSpan="6">No Stage 3 referrals available.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default Stage3; // Export the component
