...
const FortnightlyProfit = () => {
  ...
  useEffect(() => {
    const loadUserData = () => {
      ...
      try {
        ...
        const profitRecords = mainUser.payoutRecords.map(({ date, amount }) => ({
          name: mainUser.name,
          date,
          profitAmount: mainUser.fortnightlyProfit,
  ...
  // Calculate totals
  const totalFortnightlyProfit = profitData.reduce((total, record) => total + record.profitAmount, 0);
  ...
  return (
    <Layout userRole="customer"> {/* Replace with actual user role if necessary */}
      <section className="fortnightly-profit">
        <h1>Fortnightly Profit Overview</h1>
        <h2>
          Total Fortnightly Profit: ${totalFortnightlyProfit.toFixed(2)} &nbsp;|&nbsp;
          Total Company Profit Due: ${totalCompanyProfitDue.toFixed(2)}
        </h2>
        <table className="profit-table">
          ...
          <tbody>
            {profitData.length > 0 ? (
              ...
              <tr>
                <td colSpan="5" style={{ textAlign: "center" }}>No fortnightly profit data available.</td>
              </tr>
            )}
          </tbody>
        </table>
...
export default FortnightlyProfit; // Export the component
